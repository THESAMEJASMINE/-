    ...
    def process_image(self, inImg):
        ...
        for i in range(len(faces)):
            ...
            if confidence[1] < 3500:
                ...
                cv2.putText(frame, '%s - %.0f' % (person, confidence[1]), (x - 10, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 1,
                            (255, 30, 0))
            ...
            if self.only == 0:
                cv2.imwrite('%s/%s.png' % ('out', "识别"), frame)
                self.only = 1
                print("INFO--成功保存图片.....")
